...
def find_genus(record, genus):
    ...
    matched_grids['annotation_path'] = matched_grids['grid_path'] + '_grid_' + matched_grids[2] + '.xml'
...
def process_grids(matched_grids, grid_path, output_path, image_dir ,annotation_path):
    ...
    for idx, row in matched_grids.iterrows():
        ...
        annotation_source_path = os.path.join(grid_source_folder_path, row['annotation_path'])
        grid_source_path, annotation_source_path, grid_dest_path = \
            test_file_existance(grid_source_path, annotation_source_path, grid_dest_path)
...
if __name__ == '__main__':
    ...
    process_grids(matched_grids,grid_path,out_path,image_path,'')
